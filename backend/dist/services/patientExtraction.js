...
exports.extractPatientFromHistory = extractPatientFromHistory;
...
function extractPatientFromHistory(history) {
    const allUser = history
        .filter((m) => m.role === "user")
        .map((m) => m.content)
        .join("\n");
    const firstName = allUser.match(/\b(?:i am|i'm|my name is)\s+([a-zA-Z]+)\b/i)?.[1] ?? "";
    const twoWordNameMatch = allUser.match(/\b(?:i am|i'm|my name is)\s+([a-zA-Z]+)\s+([a-zA-Z]+)\b/i);
    let email = allUser.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi)?.[0] ?? "";
    ...
    if (!dob) {
        ...
        // YYYY/MM/DD
        const ymdSlash = allUser.match(/\b\d{4}\/\d{1,2}\/\d{1,2}\b/)?.[0] ?? "";
